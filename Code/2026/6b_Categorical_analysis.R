...
## Secondary analysis of categorical exposure
## Date: 4 June 2025
...
msm.formula <- "Y ~ cum_exp_150 + cum_exp_300 + max_exp + I(max_exp^2) + I(max_exp^3) + cum_exp_150:max_exp + cum_exp_300:max_exp"
...
ranger_64 <- create.Learner("SL.ranger", params = list(num.trees = 64))

SLlib <- list(Q=c("SL.mean","SL.glm","SL.gam"),
              g=c("SL.mean","SL.glm","SL.gam",ranger_64$names))

######################################################################################
# 5. Load data
#-------------------------------------------------------------------------------------

analysis_data <- readRDS(paste0(workdir,"Data/categorical analysis data - wide form 20240827.rds"))
...
saveRDS(sum,file=paste0(workdir,"Results/ltmle-cat-summary-",args[1],".rds"))
