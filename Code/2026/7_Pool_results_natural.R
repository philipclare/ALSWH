...
get_res_bin <- function (summary) {
  ...
  pr1 <- do.call(rbind,lapply(seq(0,7), function (x) {
  ...
  pr2 <- car::deltaMethod(coef,"exp(Intercept+control)/(1+exp((Intercept)+control))",vcov=cov.mat_ic)
  
  pr <- rbind(pr1,pr2)
  
  rr1 <- do.call(rbind,lapply(seq(0,7), function (x) {
    car::deltaMethod(coef,"(exp(Intercept+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`)/(1+exp((Intercept)+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`))) / 
                     (exp((Intercept)+7*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+7*7*`cum_exp:max_exp`)/(1+exp((Intercept)+7*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+7*7*`cum_exp:max_exp`)))",vcov=cov.mat_ic)
  }))
  
  rr1
  
  rr2 <- do.call(rbind,lapply(seq(0,7), function (x) {
    car::deltaMethod(coef,"(exp(Intercept+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`)/(1+exp((Intercept)+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`))) / 
                     (exp(Intercept+control)/(1+exp((Intercept)+control)))",vcov=cov.mat_ic)
  }))
  
  rr2
  
  rd1 <- do.call(rbind,lapply(seq(0,7), function (x) {
    car::deltaMethod(coef,"(exp(Intercept+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`)/(1+exp((Intercept)+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`))) - 
                     (exp((Intercept)+7*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+7*7*`cum_exp:max_exp`)/(1+exp((Intercept)+7*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+7*7*`cum_exp:max_exp`)))",vcov=cov.mat_ic)
  }))
  ...
  rd2 <- do.call(rbind,lapply(seq(0,7), function (x) {
    car::deltaMethod(coef,"(exp(Intercept+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`)/(1+exp((Intercept)+x*cum_exp+7*max_exp+7^2*`I(max_exp^2)`+7^3*`I(max_exp^3)`+x*7*`cum_exp:max_exp`))) - 
                     (exp(Intercept+control)/(1+exp((Intercept)+control)))",vcov=cov.mat_ic)
  }))
...
meld_res <- function(res_list) {
  ...
  num_estimates <- ncol(res_list)/2
  
  num <- seq(0,num_estimates-1)
  
  res <- t(do.call(rbind,mi.meld(q = res_list[,c(1:num_estimates)],
                                 se = res_list[,c((num_estimates+1):(num_estimates*2))])))
...
sum_pr <- lapply(seq(1,25), function (x) {
  res <- readRDS(paste0(workdir,"Results/Katana Output/Natural/ltmle-bin-summary-",x,".rds")) 
  res[[3]] <- nrow(readRDS(paste0(workdir,"Data/primary analysis data - 20240827.rds"))[[x]])
  res
})
...
rr_pr_natural <- do.call(rbind,lapply(res_pr, function (x) {
  input <- x[[3]]
  input <- c(as.vector(input[,1]),as.vector(input[,2]))
}))
...
rd_pr_natural <- do.call(rbind,lapply(res_pr, function (x) {
  input <- x[[5]]
  input <- c(as.vector(input[,1]),as.vector(input[,2]))
}))
...
res_mn_pr$group <- factor(c(rep("Primary",8),"Natural"))
...
res_rr_pr_natural <- meld_res(rr_pr_natural)
res_rr_pr_natural$label <- paste0(sprintf('%.1f',res_rr_pr_natural$est),"\n(",sprintf('%.1f',res_rr_pr_natural$lb),", ",sprintf('%.1f',res_rr_pr_natural$ub),")")
...
res_rd_pr_natural <- meld_res(rd_pr_natural)
res_rd_pr_natural$label <- paste0(sprintf('%.1f%%',res_rd_pr_natural$est*100),"\n(",sprintf('%.1f%%',res_rd_pr_natural$lb*100),", ",sprintf('%.1f%%',res_rd_pr_natural$ub*100),")")
...
saveRDS(res_rr_pr_natural,file=paste0(workdir,"Results/Processed/rrs - natural - binary.rds"))
saveRDS(res_rd_pr,file=paste0(workdir,"Results/Processed/rds - binary.rds"))
saveRDS(res_rd_pr_natural,file=paste0(workdir,"Results/Processed/rds - natural - binary.rds"))
